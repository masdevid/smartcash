...
Deskripsi: Fixed utilities dengan simplified progress delegation untuk avoid weak reference error
"""
...
class SimpleProgressDelegate:
    """Simple progress delegate untuk UI components tanpa weak reference"""
    
    def __init__(self, ui_components: Dict[str, Any]):
        self.ui_components = ui_components
        
    def next_step(self, step_name: str, message: str) -> None:
        """Move ke step berikutnya dengan message - delegasi ke progress tracker"""
        self._safe_update_progress('overall', 0, message)
        self._safe_log(f"📋 {step_name}: {message}")
    
    def update_current_step(self, progress: int, message: str) -> None:
        """Update current step progress - delegasi ke progress tracker"""
        self._safe_update_progress('current', progress, message)
        progress % 25 == 0 and progress > 0 and self._safe_log(f"⏳ Progress: {progress}%")
    
    def _safe_update_progress(self, level: str, progress: int, message: str) -> None:
        """Safe update progress dengan fallback"""
        update_fn = self.ui_components.get('update_progress')
        update_fn and update_fn(level, progress, message)
