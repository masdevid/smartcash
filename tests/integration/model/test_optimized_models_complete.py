"""
/Users/masdevid/Projects/smartcash/tests/integration/model/test_optimized_models_complete.py

Pengujian untuk memverifikasi bahwa semua model optimasi dapat dibangun dengan benar
menggunakan pretrained model dari Google Drive dengan mocking yang komprehensif.
"""
    ...
    def test_build_all_optimized_models(self):
        ...
        for model_type, config in tqdm(OPTIMIZED_MODELS.items(), desc="Menguji model optimasi"):
            with self.subTest(model_type=model_type):
                ...
                # Patch untuk check_pretrained_model_in_drive
                with patch('smartcash.model.utils.pretrained_model_utils.check_pretrained_model_in_drive') as mock_check_drive:
                    # Setup mock untuk check_pretrained_model_in_drive
                    mock_check_drive.return_value = None  # Tidak ada model di drive, gunakan default pretrained
                    
                    # Patch untuk _build_backbone
                    with patch.object(ModelManager, '_build_backbone', autospec=True) as mock_build_backbone:
                        # Setup dummy backbone
                        dummy_backbone = DummyBackbone()
                        mock_build_backbone.return_value = dummy_backbone
                        
                        # Patch untuk load_pretrained_model
                        with patch('smartcash.model.utils.pretrained_model_utils.load_pretrained_model') as mock_load_pretrained:
                            # Setup mock untuk load_pretrained_model
                            mock_load_pretrained.return_value = None  # Tidak perlu mengembalikan apa-apa
                            
                            try:
                                # Buat model manager dengan model_type yang sesuai
                                # testing_mode=True untuk menghindari loading pretrained model
                                ...
                                # Atur backbone secara manual sebelum build_model
                                model_manager.backbone = dummy_backbone
