...
Deskripsi: Fixed config operations dengan parameter alignment dan UI consistency
"""
...
# One-liner config extractors dengan aligned parameters
...
def create_aug_config(config: Dict[str, Any]) -> AugConfig:
    """Create AugConfig dengan aligned parameters"""
...
def extract_ui_config(ui_components: Dict[str, Any]) -> Dict[str, Any]:
    """Extract config dari UI dengan parameter alignment yang konsisten"""
    config = ui_components.get('config', {})
    base_data_dir = get_best_data_location()
    
    # Extract augmentation types dari UI - multiple fallback strategies
    selected_types = _extract_augmentation_types_safe(ui_components)
    
    # Extract basic parameters dengan widget value extraction
    ...
    return {
        ...
        'augmentation': {
            ...
            **advanced_params  # Include UI parameters
        },
        'preprocessing': {
...
def _extract_augmentation_types_safe(ui_components: Dict[str, Any]) -> List[str]:
    """Safe extraction augmentation types dengan multiple fallback"""
    # Strategy 1: Direct augmentation_types widget
    ...
    # Strategy 2: aug_options container
    ...
    # Strategy 3: types_widget alternative name
...
def apply_config_to_ui(ui_components: Dict[str, Any], config: Dict[str, Any]) -> None:
    """Apply config back ke UI widgets dengan parameter alignment"""
    aug_config = config.get('augmentation', {})
...
# Auto-detect config dengan aligned parameters
auto_detect_config = lambda: {
    ...
    'augmentation': {
        ...
        # UI parameters dengan research-friendly defaults
...
# One-liner config mergers dengan parameter preservation
...
validate_ui_parameters = lambda ui_components: all([
    _get_widget_value_safe(ui_components, 'num_variations', 0) > 0,
    _get_widget_value_safe(ui_components, 'target_count', 0) > 0,
    len(_extract_augmentation_types_safe(ui_components)) > 0
])
