...
def run_batch_installation(packages: List[str], ui_components: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    # Log info
    if logger: logger.info(f"🚀 Memulai instalasi {len(packages)} package...")
    ...
    for i, package in enumerate(tqdm(packages, desc="Instalasi Package")):
        ...
        # Log info
        if logger: logger.info(f"📦 Menginstall {package}...")
        ...
        try:
            ...
            if result.returncode == 0:
                # Sukses
                stats['success'] += 1
                if logger: logger.info(f"✅ Berhasil menginstall {package}")
            else:
                # Gagal
                stats['failed'] += 1
                error_msg = result.stderr.strip() or "Unknown error"
                stats['errors'].append((package, error_msg))
                if logger: logger.error(f"❌ Gagal menginstall {package}: {error_msg}")
        except Exception as e:
            # Error
            stats['failed'] += 1
            stats['errors'].append((package, str(e)))
            if logger: logger.error(f"❌ Error saat menginstall {package}: {str(e)}")
    ...
    # Log ringkasan
    if logger: 
        logger.info(f"✅ Instalasi selesai: {stats['success']}/{stats['total']} berhasil, {stats['failed']} gagal")
        logger.info(f"⏱️ Waktu: {stats['duration']:.1f} detik")
