    ...
    **layout_kwargs
    ...
    def create_layout():
        ...
        if l_type == LayoutType.COLUMN:
            ...
            vbox = widgets.VBox([])
            vbox.layout = widgets.Layout(display='flex', flex_flow='column')
            for k, v in column_layout.items():
                setattr(vbox.layout, k, v)
            return vbox
            ...
            hbox = widgets.HBox([])
            hbox.layout = widgets.Layout(display='flex')
            for k, v in row_layout.items():
                setattr(hbox.layout, k, v)
            return hbox
            ...
            grid.layout = widgets.Layout(display='grid')
            for k, v in layout_props.items():
                setattr(grid.layout, k, v)
            return grid
    ...
    container.layout = widgets.Layout(width='100%', margin=container_margin, padding='0', overflow='visible')
