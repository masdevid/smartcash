    ...
    def rename_invalid_files(self, file_paths: List[Path], 
        ...
        for i, file_path in enumerate(file_paths):
            try:
                ...
                if progress_callback and i % max(1, len(file_paths) // 10) == 0:
                    batch_num = (i // max(1, len(file_paths) // 10)) + 1
                    progress_callback('current', i + 1, len(file_paths), f"Validating batch {batch_num}/10...")
