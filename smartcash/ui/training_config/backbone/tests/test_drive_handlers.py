...
Deskripsi: Test untuk drive_handlers backbone
"""
...
class TestBackboneDriveHandlers(unittest.TestCase):
    """
    Test untuk drive_handlers backbone
    """
    ...
    @patch('smartcash.ui.training_config.backbone.handlers.drive_handlers.update_sync_status_only')
    ...
    def test_sync_to_drive_success(self, mock_update_status, mock_get_config, mock_get_env, mock_is_colab):
        ...
        self.assertTrue(success)
        mock_config_manager.sync_to_drive.assert_called_once_with('model')
        mock_update_status.assert_any_call(self.mock_ui_components, "Konfigurasi backbone berhasil disinkronkan ke Google Drive", 'success')
    ...
    def test_sync_to_drive_failure(self, mock_update_status, mock_get_config, mock_get_env, mock_is_colab):
        ...
        mock_config_manager.sync_to_drive.assert_called_once_with('model')
        mock_update_status.assert_any_call(self.mock_ui_components, "Gagal menyinkronkan konfigurasi backbone ke Google Drive: Sync failed", 'error')
    ...
    def test_sync_from_drive_success(self, mock_update_status, mock_update_ui, mock_get_config, mock_get_env, mock_is_colab):
        ...
        mock_config_manager.sync_with_drive.assert_called_once_with('model_config.yaml', sync_strategy='drive_priority')
        ...
        mock_update_status.assert_any_call(self.mock_ui_components, "Konfigurasi backbone berhasil disinkronkan dari Google Drive", 'success')
    ...
    def test_sync_with_drive_success(self, mock_update_status, mock_get_config, mock_is_colab):
        ...
        with patch('builtins.__import__', side_effect=ImportError):
            ...
            config_with_model = {'model': self.mock_config['model']}
            
            result = sync_with_drive(config_with_model, self.mock_ui_components)
            
            self.assertEqual(result, self.mock_config)
            mock_config_manager.save_module_config.assert_called_once_with('model', config_with_model)
            mock_config_manager.sync_to_drive.assert_called_once_with('model')
