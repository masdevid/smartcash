...
class TrainingUIModule(BaseUIModule, OperationMixin, ButtonHandlerMixin):
    """
    ...
    def _setup_chart_updaters(self) -> None:
        ...
        try:
            ...
            def update_loss_chart(loss_data: Dict[str, Any]) -> None:
                try:
                    ...
                    if loss_chart and hasattr(loss_chart, 'add_data'):
                        ...
                        self.logger.debug(f"📊 Loss chart updated: epoch {loss_data.get('epoch', 0)}")
                except Exception as e:
                    self.logger.warning(f"Failed to update loss chart: {e}")
            ...
            def update_map_chart(map_data: Dict[str, Any]) -> None:
                try:
                    ...
                    if map_chart and hasattr(map_chart, 'add_data'):
                        ...
                        self.logger.debug(f"📊 mAP chart updated: epoch {map_data.get('epoch', 0)}")
                except Exception as e:
                    self.logger.warning(f"Failed to update mAP chart: {e}")
            ...
            self.logger.debug("✅ Chart updaters configured for live monitoring")
            
        except Exception as e:
            self.logger.warning(f"Failed to setup chart updaters: {e}")
    ...
    def _get_backbone_configuration(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if backbone_module and hasattr(backbone_module, 'get_current_config'):
                ...
                self.logger.debug("✅ Backbone configuration retrieved")
                return backbone_config
            else:
                self.logger.warning("⚠️ Backbone module not available")
                return None
                
        except Exception as e:
            self.logger.warning(f"⚠️ Error getting backbone config: {e}")
            return None
