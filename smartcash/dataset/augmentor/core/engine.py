...
class AugmentationEngine:
    ...
    def _execute_augmentation(self, files: List[str], target_split: str, 
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            # Batch size for progress updates (reduce verbosity)
            progress_batch_size = max(1, total_files // 10)  # Update every 10%
            ...
            for future in as_completed(future_to_file):
                ...
                # Batched progress reporting (reduce verbosity)
                if processed_files % progress_batch_size == 0 or processed_files == total_files:
                    progress_pct = (processed_files / total_files) * 100
                    batch_num = (processed_files // progress_batch_size) + (1 if processed_files % progress_batch_size else 0)
                    self._report_progress("current", processed_files, total_files, 
                                        f"Batch {batch_num}/10 - Generated {total_generated} files ({progress_pct:.1f}%)", progress_callback)
    ...
    def _augment_single_file(self, file_path: str, pipeline, output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            generated_count = 0
            variation_errors = []  # Collect errors for batched logging
            ...
            for var_idx in range(1, self.num_variations + 1):
                try:
                    ...
                    variation_errors.append(f"var{var_idx}: {str(e)}")
                    continue
            
            # Batch log variation errors to reduce verbosity
            if variation_errors:
                error_summary = f"{len(variation_errors)} variation errors"
                if len(variation_errors) <= 3:
    ...
    def _log_detailed_summary(self, summary: Dict[str, Any]):
        ...
        self.logger.info(
            f"📊 Summary: {input_info['processed_files']}/{input_info['source_files']} files processed ({input_info['processing_rate']}), "
