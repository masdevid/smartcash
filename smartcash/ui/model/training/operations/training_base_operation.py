    ...
    def clear_operation_logs(self) -> None:
        ...
        try:
            ...
            if operation_container and hasattr(operation_container, 'clear_logs'):
                ...
                self.logger.debug("✅ Training operation logs cleared")
            elif operation_container and hasattr(operation_container, 'clear'):
                operation_container.clear()
                self.logger.debug("✅ Training operation container cleared")
            else:
                self.logger.debug("⚠️ No clear method available on training operation container")
        except Exception as e:
            self.logger.error(f"Failed to clear training operation logs: {e}")
