"""
File: smartcash/ui/dataset/preprocess/operations/manager.py
Description: Preprocessing operation manager extending OperationHandler
"""
...
# Import preprocessing operations
...
# Import backend preprocessing service
from smartcash.ui.dataset.preprocess.services.preprocess_service import PreprocessService


class PreprocessOperationManager(OperationHandler):
    """
    Preprocessing operation manager extending OperationHandler.
    
    Features:
    - 🎯 Extends OperationHandler for consistent architecture
    - 📊 Backend service integration for all preprocessing operations
    ...
    def __init__(self, config: Dict[str, Any], operation_container: Any):
        """
        Initialize preprocessing operation manager.
    ...
    def initialize(self) -> None:
        ...
        try:
            self.log("🎯 Initializing preprocessing operation manager", 'info')
            
            # Initialize backend service
            if self._backend_service:
                self._backend_service.initialize()
            
            self.log("✅ Preprocessing operation manager initialized", 'info')
    ...
    def get_operations(self) -> Dict[str, Callable]:
        ...
        """
        return {
            'preprocess': self.execute_preprocess,
    ...
    def execute_preprocess(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._current_operation = PreprocessingOperation.PREPROCESS
            
            # Use provided config or instance config
            operation_config = config or self.config
            
            self.log("🚀 Starting preprocessing operation", 'info')
            ...
            try:
                ...
                result = self._backend_service.preprocess_dataset(operation_config)
                
                # Log result
                if result.get('success', False):
                    self.log(SUCCESS_MESSAGES['preprocessing_complete'], 'info')
                    
                    # Log processing statistics
                    stats = result.get('stats', {})
                    if 'processed_files' in stats:
                        self.log(f"📊 Processed {stats['processed_files']} files", 'info')
                    if 'processed_splits' in result:
                        splits = ', '.join(result['processed_splits'])
                        self.log(f"📁 Processed splits: {splits}", 'info')
    ...
    def execute_check(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            self._is_processing = True
            self._current_operation = PreprocessingOperation.CHECK
            ...
            try:
                # Execute via backend service
                result = self._backend_service.get_preprocessing_status(operation_config)
    ...
    def execute_cleanup(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            self._is_processing = True
            self._current_operation = PreprocessingOperation.CLEANUP
            ...
            try:
                # Execute via backend service
                result = self._backend_service.cleanup_preprocessing_files(operation_config)
    ...
    def cleanup(self) -> None:
        ...
        try:
            self.log("Cleaning up preprocessing operation manager", 'info')
