...
- [✅] **medium priority** Add visualization debug for bbox alignment - Implemented comprehensive ASCII grid visualization debug system in MapDebugLogger with bbox alignment analysis. Features 20x20 ASCII grids showing prediction/target centers (P/T), overlap regions (*), and bbox edges (p/t). Integrated into YOLOv5MapCalculator and BatchProcessor for automatic coordinate mismatch detection. Provides visual debugging without requiring image files, calculating IoU values, center distances, and detecting coordinate system mismatches (pixel vs normalized) (08/05/2025)
- [✅] **documentation** Create investigation documentation on overlap between training_pipeline, training/pipeline and training/phase - Created markdown document analyzing the roles of each component, identifying areas of overlap, and proposing a path toward simplification. (08/06/2025)