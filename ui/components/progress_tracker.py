...
"""
# Usage examples
    # Example 1: Single progress tracker
    single_tracker = create_single_progress_tracker("Download")
    single_tracker.show()
    
    # Example 2: Dual progress tracker
    dual_tracker = create_dual_progress_tracker("Processing")
    dual_tracker.show()
    
    # Example 3: Triple progress tracker dengan callbacks (WAJIB provide steps)
    triple_tracker = create_triple_progress_tracker(
        "Training", 
        steps=['prepare', 'train', 'validate', 'save'],
        step_weights={'prepare': 15, 'train': 60, 'validate': 15, 'save': 10}
    )
    
    # Register callbacks
    def on_progress(level, progress, message):
        print(f"Progress update: {level} = {progress}% ({message})")
    
    def on_step_done(step_name, step_index):
        print(f"Step completed: {step_name} (index: {step_index})")
    
    def on_operation_complete():
        print("Operation completed!")
    
    triple_tracker.on_progress_update(on_progress)
    triple_tracker.on_step_complete(on_step_done)
    triple_tracker.on_complete(on_operation_complete)
    ...
    custom_config = ProgressConfig(
        ...
        step_weights={'init': 20, 'process': 70, 'finalize': 10},
        auto_advance=True,
        auto_hide_delay=5.0
    )
    custom_tracker = create_flexible_tracker(custom_config)
"""