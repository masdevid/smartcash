...
Deskripsi: Fixed core utilities dengan smart image detection untuk resolve "Tidak ada file gambar ditemukan"
"""
...
# =============================================================================
# ENHANCED PATH OPERATIONS - Fixed untuk detect images di berbagai lokasi
# =============================================================================
...
def resolve_drive_path(path: str) -> str:
    ...
    # Return path as-is for absolute paths
    return path
...
def find_dataset_directories(base_path: str) -> List[str]:
    ...
    # Standard YOLO structure patterns
    yolo_patterns = [
        resolved_path,                          # Direct path
        os.path.join(resolved_path, 'data'),    # data subdirectory
        os.path.join(resolved_path, 'dataset'), # dataset subdirectory
        os.path.join(resolved_path, 'images'),  # images subdirectory
    ]
    
    # Add split directories
    for split in ['train', 'valid', 'test', 'val']:
        yolo_patterns.extend([
            os.path.join(resolved_path, split),
            os.path.join(resolved_path, split, 'images'),
            os.path.join(resolved_path, 'data', split),
            os.path.join(resolved_path, 'data', split, 'images')
        ])
    
    # Filter existing directories
    for pattern in yolo_patterns:
        if os.path.exists(pattern) and os.path.isdir(pattern):
            dataset_dirs.append(pattern)
    
    return list(set(dataset_dirs))  # Remove duplicates
...
def smart_find_images(base_path: str, extensions: List[str] = None) -> List[str]:
    ...
    dataset_dirs = find_dataset_directories(base_path)
...
# =============================================================================
# ENHANCED FILE OPERATIONS - Fixed image detection
# =============================================================================
...
# =============================================================================
# IMAGE OPERATIONS - Enhanced dengan validation
# =============================================================================
...
# =============================================================================
# ENHANCED DATASET DETECTION - Fixed untuk detect images dengan smart search
# =============================================================================
...
def detect_structure(data_dir: str) -> Dict[str, Any]:
    ...
    for dir_path in dataset_dirs:
        ...
        if dir_images:
            image_locations.append({
                ...
                'sample_files': dir_images[:3]  # Show first 3 files for debugging
            })
    ...
    return {
        ...
        'structure_type': 'standard_yolo' if has_yolo else f'mixed_structure_{len(splits)}_splits' if splits else 'flat_structure',
        'splits_detected': splits, 
        'image_locations': image_locations,
        'recommendations': [
            f'✅ Ditemukan {len(images)} gambar di {len(image_locations)} lokasi' if images else 
            '❌ Tidak ada gambar ditemukan - periksa path dan format file',
...
# =============================================================================
# BATCH PROCESSING - Enhanced dengan better error handling
# =============================================================================
...
# =============================================================================
# CONFIG EXTRACTION - Enhanced dengan better defaults
# =============================================================================
...
def get_best_data_location() -> str:
    ...
    for path in search_paths:
        if smart_find_images(path):  # Has images
            return path
        elif path_exists(path):  # Directory exists
            return path
