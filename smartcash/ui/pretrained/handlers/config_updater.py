        ...
        def wrapper(*args, **kwargs) -> T:
            try:
                ...
                if logger:
                    ...
                    logger = get_module_logger('smartcash.ui.pretrained.handlers.config_updater')
                    logger.error(error_msg, exc_info=True)
...
def update_pretrained_ui(
    ...
    if 'models_dir_input' in ui_components and 'models_dir' in pretrained_config:
        ...
        _log_debug(logger, f"Updated models_dir: {pretrained_config['models_dir']}")
    ...
    if 'drive_models_dir_input' in ui_components and 'drive_models_dir' in pretrained_config:
        ...
        _log_debug(logger, f"Updated drive_models_dir: {pretrained_config['drive_models_dir']}")
    ...
    if 'pretrained_type_dropdown' in ui_components and 'pretrained_type' in pretrained_config:
        ...
        _log_debug(logger, f"Updated pretrained_type: {pretrained_config['pretrained_type']}")
    ...
    if 'sync_drive_checkbox' in ui_components and 'sync_drive' in pretrained_config:
        ...
        _log_debug(logger, f"Updated sync_drive: {pretrained_config['sync_drive']}")
    
    _log_info(logger, "UI berhasil diupdate dengan konfigurasi yang dimuat")
...
def reset_pretrained_ui(
    ...
    try:
        ...
        update_pretrained_ui(ui_components, default_config, logger)
        
        _log_info(logger, "UI berhasil direset ke nilai default")
        
    except ImportError as e:
        error_msg = f"Gagal mengimpor modul defaults: {str(e)}"
        _log_error(logger, error_msg, exc_info=True)
...
def apply_config_to_ui(
    ...
    try:
        ...
        if config_key == 'model_urls' and isinstance(config_value, dict):
            ...
            for model_name, url in config_value.items():
                ...
                if ui_key and ui_key in ui_components:
                    ...
                    _log_debug(logger, f"Applied {model_name} URL: {url}")
                    updated = True
            return updated
        ...
        if not ui_key or ui_key not in ui_components:
            _log_warning(logger, f"UI component tidak ditemukan untuk key: {config_key}")
            return False
        
        # Apply value ke UI component
        ui_components[ui_key].value = config_value
        _log_debug(logger, f"Applied {config_key}: {config_value}")
        return True
