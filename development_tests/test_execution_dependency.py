#!/usr/bin/env python3
"""
Test execution script for cell_1_3_dependency.py

This script tests the dependency UI initialization and ensures:
...
def test_dependency_execution():
    """Test dependency cell execution with proper UI display and logging management"""
    
    print("🧪 Testing cell_1_3_dependency.py execution...")
    print("=" * 60)
    ...
    try:
        ...
        with capture_stdout_stderr() as (stdout_capture, stderr_capture):
            ...
            # Import and initialize the dependency UI
            ...
            ui_result = initialize_dependency_ui()
        ...
        if isinstance(ui_result, dict) and 'ui_handler' in ui_result:
            print("   Found UI handler - testing operation availability...")
            handler = ui_result['ui_handler']
            if hasattr(handler, 'operation_manager'):
                print("   ✅ Operation manager available")
            else:
                print("   ⚠️  Operation manager not found")
        
        print("✅ Dependency execution test completed")
        return True
...
def test_dependency_reinitialization():
    """Test that reinitializing doesn't create duplicate UI"""
    
    print("\n🔄 Testing dependency reinitialization...")
    
    try:
        from smartcash.ui.setup.dependency.dependency_initializer import initialize_dependency_ui
        
        # First initialization
        ui_result1 = initialize_dependency_ui()
        
        # Second initialization (should reuse existing)
        ui_result2 = initialize_dependency_ui()
...
def test_dependency_button_functionality():
    """Test that dependency buttons are properly configured"""
    
    print("\n🔘 Testing dependency button configuration...")
    ...
    try:
        ...
        if isinstance(ui_result, dict):
            ...
            if 'action_container' in ui_result:
                print("✅ Action container found")
                action_container = ui_result['action_container']
                
                # Check if it's a widget with children (buttons)
                if hasattr(action_container, 'children'):
                    print(f"   Found {len(action_container.children)} child widgets")
                    
                    # Look for buttons with expected text
                    expected_buttons = ['Install', 'Check & Updates', 'Uninstall']
                    found_buttons = []
                ...
                print("⚠️  Action container not found in UI result")
...
if __name__ == "__main__":
    print("🚀 Starting Dependency UI Execution Tests")
    print("=" * 60)
    
    # Run main execution test
    test1_success = test_dependency_execution()
    
    # Run reinitialization test
    test2_success = test_dependency_reinitialization()
    
    # Run button functionality test
    test3_success = test_dependency_button_functionality()
