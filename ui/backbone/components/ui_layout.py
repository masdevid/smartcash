...
from smartcash.ui.components import create_header, create_responsive_container, create_responsive_two_column
...
def _setup_summary_auto_update(form_components: Dict[str, Any], summary_widget: widgets.HTML) -> None:
    ...
    # Setup widget change handlers for auto-update
    widget_updaters = [
    ...
    # Attach change handlers to each widget
    [getattr(form_components.get(widget), 'observe', lambda: None)(update_summary, names=prop_name)
     for widget, prop_name in widget_updaters if widget in form_components]