...
class FileProcessor(BaseDownloaderComponent):
    ...
    def _organize_splits_with_renaming_parallel(self, source_dir: Path, target_dir: Path, 
        ...
        with ThreadPoolExecutor(max_workers=min(self.max_workers, len(splits))) as executor:
            ...
            for i, future in enumerate(future_to_split):
                ...
                # Only log initial split processing, not individual progress updates
                if i == 0:
                    self.logger.info(f"📁 Processing {len(splits)} splits...")
                    
                # Update progress tracker (UI) but reduce logging verbosity
    ...
    def _copy_files_without_renaming(self, source_split: Path, target_split: Path) -> Dict[str, int]:
        ...
        # Copy images with batched error reporting (reduce verbosity)
        for img_file in source_images.glob('*.*'):
            if img_file.suffix.lower() in {'.jpg', '.jpeg', '.png', '.bmp'}:
                try:
                    ...
                    image_errors.append(f"{img_file.name}: {str(e)}")
        
        # Copy labels with batched error reporting
        if source_labels.exists():
            for label_file in source_labels.glob('*.txt'):
                try:
                    ...
                    label_errors.append(f"{label_file.name}: {str(e)}")
        
        # Log errors in batches to reduce verbosity
        if image_errors:
            self.logger.warning(f"⚠️ {len(image_errors)} image copy errors. First few: {'; '.join(image_errors[:3])}")
        if label_errors:
            self.logger.warning(f"⚠️ {len(label_errors)} label copy errors. First few: {'; '.join(label_errors[:3])}")
    ...
    def _process_file_batch(self, image_batch: List[Path], source_labels: Path, 
        ...
        for img_file in image_batch:
            try:
                ...
                batch_errors.append(f"{img_file.name}: {str(e)}")
        
        # Log batch errors to reduce verbosity (only if there are errors)
        if batch_errors:
            self.logger.warning(f"⚠️ {len(batch_errors)} file processing errors in batch. First few: {'; '.join(batch_errors[:2])}")
