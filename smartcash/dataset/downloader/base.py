...
Deskripsi: Base components untuk eliminasi duplikasi
"""
...
class BaseDownloaderComponent(ABC):
    """Base class untuk semua downloader components"""
...
class RequestsHelper:
    """Helper untuk HTTP requests dengan retry logic"""
...
class ValidationHelper:
    """Helper untuk common validations"""
    ...
    def validate_dataset_structure(dataset_dir: Path) -> Dict[str, Any]:
        """Quick dataset structure validation"""
        ...
        if not splits and not has_flat:
            return {'valid': False, 'message': 'Struktur dataset tidak valid'}
        
        return {'valid': True, 'splits': splits, 'has_flat': has_flat}
...
class PathHelper:
    ...
    def setup_download_paths(workspace: str, project: str, version: str) -> Dict[str, Path]:
        ...
        # Create directories
        for path in [paths['temp_dir'], paths['final_dir']]:
            FileHelper.ensure_directory(path)
        
        return paths


# Factory untuk shared components
...
# Export
__all__ = [
    'BaseDownloaderComponent', 'ProgressTracker', 'RequestsHelper', 
    'ValidationHelper', 'FileHelper', 'PathHelper',
    'create_progress_tracker', 'create_validation_helper',
    'create_file_helper', 'create_path_helper'
]