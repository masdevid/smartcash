...
from smartcash.dataset.downloader.base import BaseDownloaderComponent, ValidationHelper, FileHelper, DirectoryManager
...
class DownloadService(BaseDownloaderComponent):
    ...
    def __init__(self, config: Dict[str, Any], logger=None, logger_bridge=None):
        ...
        Args:
            ...
            logger_bridge: LoggerBridge instance for centralized logging (ignored for now)
        """
        # BaseDownloaderComponent only accepts logger parameter
        super().__init__(logger=logger)
        
        # Store logger_bridge for future use if needed
        ...
        self.chunk_size = config.get('chunk_size', 1024*64)  # 64KB for optimal performance
    ...
    def _download_dataset_file(self, download_url: str, paths: Dict[str, Path]) -> None:
        ...
        # Reset progress milestone flags for new download
        for attr in ['_progress_10', '_progress_30', '_progress_50', '_progress_70', '_progress_90']:
            if hasattr(self, attr):
                delattr(self, attr)
                
        if self.progress_tracker:
            self.progress_tracker.update_stage(10, f"📥 Starting download (chunk: {self.chunk_size//1024}KB)...")
            ...
            with open(paths['temp_zip'], 'wb') as f:
                for chunk in response.iter_content(chunk_size=self.chunk_size):
                    if chunk:
                        ...
                        # Update progress tracker only at key milestones (reduce verbosity)
                        if total_size > 0 and self.progress_tracker:
                            progress = int((downloaded / total_size) * 80) + 10  # 10-90%
                            
                            # Only update progress at significant milestones to reduce noise
                            milestone_reached = any([
                                progress >= 10 and not hasattr(self, '_progress_10'),
                                progress >= 30 and not hasattr(self, '_progress_30'), 
                                progress >= 50 and not hasattr(self, '_progress_50'),
                                progress >= 70 and not hasattr(self, '_progress_70'),
                                progress >= 90 and not hasattr(self, '_progress_90')
                            ])
                            ...
                            if milestone_reached:
                                ...
                                if progress >= 10: self._progress_10 = True
                                if progress >= 30: self._progress_30 = True  
                                if progress >= 50: self._progress_50 = True
                                if progress >= 70: self._progress_70 = True
                                if progress >= 90: self._progress_90 = True
    ...
    def _extract_dataset(self, paths: Dict[str, Path]) -> None:
        ...
        try:
            ...
            with zipfile.ZipFile(paths['temp_zip'], 'r') as zip_ref:
                ...
                # Extract dengan minimal progress updates (reduce verbosity)
                batch_size = max(1, total_files // 5)  # Only 5 progress updates max
                
                for i, file in enumerate(file_list):
                    zip_ref.extract(file, paths['extract_dir'])
                    
                    # Only update progress at 20%, 40%, 60%, 80%, 90% milestones
...
def create_download_service(config: Dict[str, Any], logger=None) -> Optional[DownloadService]:
    ...
    try:
        ...
        logger.info(f"✅ Download service created with {merged_config['max_workers']} workers")
        return service
