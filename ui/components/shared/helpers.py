"""
File: smartcash/ui/components/shared/helpers.py
...
from smartcash.ui.components.shared.layouts import STANDARD_LAYOUTS
...
def run_async_task(task_func, on_complete=None, on_error=None, with_output=None):
    """
    Jalankan task secara asinkron dalam thread terpisah.
...
def create_button_group(buttons, layout=None):
    ...
    for label, style, icon, callback in buttons:
        btn = widgets.Button(
            ...
            icon=icon,
            layout=STANDARD_LAYOUTS['button_small']
        )
    ...
    return widgets.HBox(btn_widgets, layout=layout or STANDARD_LAYOUTS['hbox'])
